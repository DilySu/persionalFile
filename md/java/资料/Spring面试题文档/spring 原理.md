...
7种：Required、Requires_new、nested、Support、Not_Support、Never、Mandatory

> 事务嵌套时怎么办？
>
> A调B方法， AB都有事务，且传播特性不同，A有异常B怎么办，B有异常A怎么办？
>
> 总： 事务的传播特性指的是不同方法的嵌套调用过程中，事务应该如何进行处理，是使用同一个事务还是不同事务，当出现异常时会提交还是回滚，两个方法之间的相互影响，在日常工作中使用比较多的是 required、requires_new、nested
>
> 分：1、事务的不同分类可分为三类：支持当前事务，不支持当前事务，嵌套事务
>
> 2、外层若是 required，内层是 required、requires_new、nested
>
> 2、外层若是 requires_new，内层是 required、requires_new、nested
>
> 2、外层若是 nested，内层是 required、requires_new、nested
...
> 是：异常统一外层处理
>
> 不是： 内层方法可能会影响到外层，但是外层不会影响到内层方法，个别不同（nested，保存点）
